  ...
  'relative inline-flex items-center justify-center gap-2 font-semibold transition-all focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 overflow-hidden [&_svg]:pointer-events-none',
  {
    variants: {
      variant: {
        ...
        outline:
          'border-2 border-border bg-transparent text-foreground hover:border-primary',
        ...
        // "gradient" (roxo/rosa no componente original) foi removida: não
        // é usada em nenhum lugar do site e carregava uma paleta que não
        // tem nada a ver com a identidade Gracie Barra.
      ...
      radius: {
        // "default" segue o --radius do site (cantos quase retos, ver
        // app/globals.css) em vez do rounded-full do componente original —
        // consistência de forma com o resto da identidade Gracie Barra.
        default: 'rounded-sm',
        sm: 'rounded-lg',
        lg: 'rounded-4xl',
        none: 'rounded-none',
...
const variantColors: Record<
  ...
  VariantColorsType
> = {
  // Recolorido para a identidade Gracie Barra: parte do preto/vermelho
  // escuro do tema e revela o vermelho da marca no hover, em vez do
  // verde/lima padrão do componente original.
  default: { fromBg: '#100c0c', toBg: '#9a1e20', stroke: '#9a1e20' },
...
const WavyButton = React.forwardRef<HTMLButtonElement, WavyButtonProps>(
  (
    ...
    // O SVG fica como irmão do Component, fora dele — quando asChild=true,
    // Component vira o Radix Slot, que exige exatamente UM filho direto
    // (o elemento que o chamador passou, ex: um <a>). Colocar o SVG dentro
    // de Component junto de children quebra o Slot (bug real do
    // componente original do ScrollX UI, corrigido aqui).
    return (
      <span
